...
console.log("Value: "+dvalue);
      ...
      if(dvalue>=total){
        ...
        $("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+dvalue+" RPG unable to send "+total+" RPG").fadeOut().fadeIn();
        thisbtn.attr('disabled',false);
      }
    ...
    $.ajax ({
      ...
      complete: function(data, status) {
                if (!isNaN(data.responseText))
                    $("#walletBalance").html(data.responseText+" RPG"); 
                else
                    $("#walletBalance").html("0.00 RPG"); 
        $("#walletLoader").addClass("hidden");
                console.log(data);
      }
    });
  ...
  $("#redeemFromBtn").click(function(){
    ...
    listUnspentCryptoidinfo_RPG(redeem);
  ...
  function listUnspentCryptoidinfo_RPG(redeem) {
